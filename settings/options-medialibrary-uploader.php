 ...
 *
 * Slightly modified for use within thte options.
 */
if ( is_admin() ) {
	
	// Load additional css and js for image uploads on the Options Framework page
	$wplannerfb_page = 'toplevel_page_wplannerfb';
	add_action( "admin_print_styles", 'wplanner_fb_mlu_css', 0 );
	add_action( "admin_print_scripts", 'wplanner_fb_mlu_js', 0 );	
}
 ...
 */
if ( ! function_exists( 'wplanner_fb_mlu_init' ) ) {

	function wplanner_fb_mlu_init () {
	
		register_post_type( 'wplannerfb', array(
			'labels' => array(
				'name' => __( 'Options Internal Container', WP_PLANNER_TEXTDOMAIN ),
			),
			'public' => true,
			'show_ui' => false,
 ...
 */
if ( ! function_exists( 'wplanner_fb_mlu_css' ) ) {

	function wplanner_fb_mlu_css () {
 ...
 */
if ( ! function_exists( 'wplanner_fb_mlu_js' ) ) {

	function wplanner_fb_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'wplannerfb-medialibrary-uploader', WP_PLANNER_URL .'settings/js/medialibrary-uploader_fb.js', array( 'jquery', 'thickbox' ) );
		wp_enqueue_script( 'wplannerfb-medialibrary-uploader' );
 ...
 *
 * Dependencies:
 * - wplanner_fb_mlu_get_silentpost()
 */
if ( ! function_exists( 'wplanner_fb_medialibrary_uploader' ) ) {

	function wplanner_fb_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '' ) {
		...
		$int = wplanner_fb_mlu_get_silentpost( $id );
		...
		$output .= '<input id="upload_' . $id . '" class="fb_upload_button button" type="button" value="' . __( 'Upload', WP_PLANNER_TEXTDOMAIN ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$title = __( 'View File', WP_PLANNER_TEXTDOMAIN );
 ...
 * Return: $_postid.
 *
 * If no "silent" post is present, one will be created with the type "wplannerfb"
 * and the post_name of "of-$_token".
 *
 * Example Usage:
 * wplanner_fb_mlu_get_silentpost ( 'wplanner_fb_logo' );
 */
if ( ! function_exists( 'wplanner_fb_mlu_get_silentpost' ) ) {

	function wplanner_fb_mlu_get_silentpost ( $_token ) {
	
		global $wpdb;
		$_id = 0;
	
		// Check if the token is valid against a whitelist.
		// $_whitelist = array( 'wplanner_fb_logo', 'wplanner_fb_custom_favicon', 'wplanner_fb_ad_top_image' );
		// Sanitise the token.
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'wplannerfb', 'post_name' => 'wplannerfb-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
 ...
 */
if ( ! function_exists( 'wplanner_fb_mlu_insidepopup' ) ) {

	function wplanner_fb_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_wplanner_fb'] ) && $_REQUEST['is_wplanner_fb'] == 'yes' ) {
		
			add_action( 'admin_head', 'wplanner_fb_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'wplanner_fb_mlu_modify_tabs' );
		}
	}
}


if ( ! function_exists( 'wplanner_fb_mlu_js_popup' ) ) {

	function wplanner_fb_mlu_js_popup () {

		$_wplanner_fb_title = $_REQUEST['wplanner_fb_title'];
		if ( ! $_wplanner_fb_title ) { $_wplanner_fb_title = 'file'; } // End IF Statement
?>
	...
	jQuery(function($) {
		...
		$( 'h3.media-title' ).each ( function () {
			...
			var new_title = current_title.replace( 'media file', '<?php echo $_wplanner_fb_title; ?>' );
			$( this ).html( new_title );
 ...
 */
if ( ! function_exists( 'wplanner_fb_mlu_modify_tabs' ) ) {

	function wplanner_fb_mlu_modify_tabs ( $tabs ) {
	
		$tabs['gallery'] = str_replace( __( 'Gallery', WP_PLANNER_TEXTDOMAIN ), __( 'Previously Uploaded', WP_PLANNER_TEXTDOMAIN ), $tabs['gallery'] );
		return $tabs;
	}
}